...
from app.api.notifications_audit import audit_router, notifications_router
from app.api.search_analytics_exports import analytics_router, exports_router
from app.api.search_analytics_exports import router as search_router
...
def create_app() -> FastAPI:
    app = FastAPI(
        ...
        version="0.1.0",
        description="Production-grade, free-first institutional web portal for patents and design registrations",
        docs_url="/docs",
        redoc_url="/redoc",
    )
    ...
    app.add_middleware(
        CORSMiddleware,
        allow_origins=app_settings.cors_origins if hasattr(app_settings, "cors_origins") else ["http://localhost:5173"],
    ...
    setup_security_middleware(app, debug=app_settings.debug)
        ...
        if request.method in ("POST", "PUT", "PATCH", "DELETE"):
            if not request.url.path.startswith("/api/v1/auth/"):
                ...
                if not validate_csrf_token(csrf_token, session_token):
                    return JSONResponse(status_code=400, content={"error": "CSRF_TOKEN_INVALID", "message": "Invalid or missing CSRF token"})
    ...
    async def portal_exception_handler(request: Request, exc: PortalError):
    ...
    app.include_router(hod_router, tags=["hod"])
    app.include_router(excel_import_router, tags=["excel-import"])
    app.include_router(upload_router, tags=["uploads"])
    app.include_router(ip_records_router, tags=["ip-records"])
    app.include_router(associations_router, tags=["associations"])
...
@app.get("/")
async def root():
    return {"name": app_settings.app_name, "version": "0.1.0", "status": "operational"}
